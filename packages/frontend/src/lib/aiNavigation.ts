...
const NAV_TARGETS: Array<AiNavigationTarget & { patterns: RegExp[] }> = [
  ...
  { path: '/sessions/scan', label: 'Scan QR', patterns: [/scan\s+qr/i, /qr\s+code/i, /open\s+camera/i, /camera\s+scan/i] },
  { path: '/biometric/attendance', label: 'Biometric Attendance', patterns: [/bio(?:metric)?\s+attendance/i, /face\s+attendance/i, /open\s+(?:face|bio)\s+camera/i] },
  { path: '/biometric/enroll', label: 'Biometric Enroll', patterns: [/enroll\s+(?:face|bio|finger)/i, /register\s+(?:face|biometric)/i] },
  { path: '/fingerprint/attendance', label: 'Fingerprint Attendance', patterns: [/finger\s*print/i, /open\s+fingerprint/i] },
