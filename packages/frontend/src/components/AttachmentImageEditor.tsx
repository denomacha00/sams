...
const AttachmentImageEditor: React.FC<AttachmentImageEditorProps> = ({ file, onCancel, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl rounded-2xl border border-line bg-surface p-5 shadow-card-hover">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-ink">Edit image before sending</h2>
            <p className="text-sm text-ink-muted">Drag over the useful part, then crop or highlight it.</p>
          </div>
          <button
            type="button"
            onClick={onCancel}
            className="rounded-lg border border-line px-3 py-2 text-sm text-ink-muted hover:bg-white/10 hover:text-ink"
          >
            Close
          </button>
        </div>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === 'crop' ? 'bg-indigo-600 text-white' : 'border border-line text-ink-muted hover:bg-white/10'}`}
          >
            Crop to selection
          </button>
          <button
            ...
            className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === 'highlight' ? 'bg-amber-600 text-white' : 'border border-line text-ink-muted hover:bg-white/10'}`}
          >
            Highlight selection
          </button>
          <button
            ...
            className="rounded-lg border border-line px-3 py-2 text-sm font-semibold text-ink-muted hover:bg-white/10 hover:text-ink"
          >
            Reset box
          </button>
        </div>
        ...
        <div
          ...
          className="relative max-h-[62vh] overflow-hidden rounded-2xl border border-line bg-black touch-none"
        >
          {objectUrl && (
            <img
              src={objectUrl}
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-ink-subtle">
            Edited images are saved as JPEG and replace the selected attachment.
          </p>
          <div className="flex justify-end gap-3">
            <button
              type="button"
              onClick={onCancel}
              className="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-ink-muted hover:bg-white/10 hover:text-ink"
            >
              Cancel
            </button>
            <button
              ...
              onClick={() => void save()}
              disabled={saving}
              className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-50"
            >
