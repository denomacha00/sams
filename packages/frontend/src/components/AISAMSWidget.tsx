      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-line bg-surface-muted rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-ink">AI-SAMS</h3>
              <p className="text-[10px] text-brand/70">Smart Assistant</p>
            </div>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            className="p-1.5 rounded-lg hover:bg-white/10 text-ink-muted hover:text-ink transition-colors"
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {QUICK_QUESTIONS.map((q) => (
                <button
                  ...
                  className="px-2.5 py-1.5 bg-indigo-500/10 border border-indigo-500/30 rounded-lg text-xs text-brand hover:bg-indigo-500/20 transition-all duration-200"
                >
                  {q}
                </button>
              ))}
        ...
        <div className="border-t border-line p-3 bg-surface rounded-b-2xl">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              ...
              disabled={loading}
              className="flex-1 input-field rounded-lg px-3 py-2 text-sm placeholder-ink-subtle focus:outline-none focus:ring-1 focus:ring-brand/30 disabled:opacity-50 transition-all"
            />
