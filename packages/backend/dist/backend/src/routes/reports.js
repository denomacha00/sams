...
const exportSchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['student', 'class', 'department', 'school']),
 ...
 */
exports.reportsRouter.get('/student/:id', (0, rbac_1.requirePermission)('view:reports'), async (req, res) => {
    try {
        const { role, sub: userId, classId: userClassId, departmentId: userDeptId } = req.user;
        const targetStudentId = req.params.id;
        // Students can only view their own report
        if (role === shared_1.UserRole.STUDENT && targetStudentId !== userId) {
        ...
        if (role === shared_1.UserRole.TEACHER) {
        ...
        const report = await reportService_1.reportService.getStudentReport(req.schoolId, targetStudentId, dateRange);
        res.status(200).json(report);
    }
    catch (err) {
        if (err instanceof errors_1.AppError)
            throw err;
        throw new errors_1.AppError(500, 'INTERNAL_ERROR', 'Failed to get student report');
    }
});
/**
...
exports.reportsRouter.get('/class/:classId', (0, rbac_1.requirePermission)('view:reports'), async (req, res) => {
    try {
        ...
        const dateRange = parseDateRange(req.query);
        const report = await reportService_1.reportService.getClassReport(req.schoolId, targetClassId, dateRange);
        res.status(200).json(report);
    }
    catch (err) {
        if (err instanceof errors_1.AppError)
            throw err;
        throw new errors_1.AppError(500, 'INTERNAL_ERROR', 'Failed to get class report');
    }
});
/**
 ...
 */
exports.reportsRouter.get('/department/:deptId', (0, rbac_1.requirePermission)('view:reports'), async (req, res) => {
    try {
        const { role, departmentId: userDeptId } = req.user;
        const targetDeptId = req.params.deptId;
        ...
        // HODs can only view reports for their own department
        if (role === shared_1.UserRole.HOD && targetDeptId !== userDeptId) {
            res.status(403).json({ error: 'Forbidden', code: 'FORBIDDEN', message: 'HODs can only view reports for their own department' });
            return;
        }
        const dateRange = parseDateRange(req.query);
        const report = await reportService_1.reportService.getDepartmentReport(req.schoolId, targetDeptId, dateRange);
        res.status(200).json(report);
    }
    catch (err) {
        if (err instanceof errors_1.AppError)
            throw err;
        throw new errors_1.AppError(500, 'INTERNAL_ERROR', 'Failed to get department report');
    }
});
/**
 ...
 */
exports.reportsRouter.get('/school', (0, rbac_1.requirePermission)('view:reports'), async (req, res) => {
    try {
        const { role } = req.user;
        // Only School Admins can access school-wide reports
        if (role !== shared_1.UserRole.SCHOOL_ADMIN && role !== shared_1.UserRole.SUPER_ADMIN) {
            res.status(403).json({ error: 'Forbidden', code: 'FORBIDDEN', message: 'Only School Admins can access school-wide reports' });
            return;
        }
        const dateRange = parseDateRange(req.query);
        const report = await reportService_1.reportService.getSchoolReport(req.schoolId, dateRange);
        res.status(200).json(report);
    }
    catch (err) {
        if (err instanceof errors_1.AppError)
            throw err;
        throw new errors_1.AppError(500, 'INTERNAL_ERROR', 'Failed to get school report');
    }
});
/**
 ...
 */
exports.reportsRouter.get('/export', (0, rbac_1.requirePermission)('view:reports'), async (req, res) => {
    ...
    try {
        ...
        switch (type) {
            case 'student':
                if (!targetId)
                    throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'targetId is required for student reports');
                reportData = await reportService_1.reportService.getStudentReport(req.schoolId, targetId, dateRange);
                break;
            case 'class':
                if (!targetId)
                    throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'targetId is required for class reports');
                reportData = await reportService_1.reportService.getClassReport(req.schoolId, targetId, dateRange);
                break;
            case 'department':
                if (!targetId)
                    throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'targetId is required for department reports');
                reportData = await reportService_1.reportService.getDepartmentReport(req.schoolId, targetId, dateRange);
                break;
            case 'school':
                reportData = await reportService_1.reportService.getSchoolReport(req.schoolId, dateRange);
                break;
        }
        const buffer = await reportService_1.reportService.exportReport(reportData, format);
        ...
        if (err instanceof errors_1.AppError)
            throw err;
        throw new errors_1.AppError(500, 'INTERNAL_ERROR', 'Failed to export report');
    }
});
/**
 ...
 */
exports.reportsRouter.get('/:reportId/export', (0, rbac_1.requirePermission)('view:reports'), async (req, res) => {
    ...
    try {
        ...
        const secureReportId = targetId
            ...
            : `${type}:${req.schoolId}`;
        const buffer = await reportService_1.reportService.exportReportById(secureReportId, format, dateRange);
        const contentType = format === 'pdf' ? 'application/pdf' : 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
        const extension = format === 'pdf' ? 'pdf' : 'xlsx';
        res.setHeader('Content-Type', contentType);
        res.setHeader('Content-Disposition', `attachment; filename="report.${extension}"`);
