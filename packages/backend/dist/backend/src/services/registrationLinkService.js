...
exports.registrationLinkService = exports.RegistrationLinkService = void 0;
...
class RegistrationLinkService {
     ...
     */
    async generateLink(creatorId, creatorRole, schoolId, departmentId, classId, options) {
        ...
        if (options && options.targetRole) {
            ...
            if (creatorRole === shared_1.UserRole.HOD) {
                if (options.targetRole !== 'TEACHER' && options.targetRole !== 'STUDENT') {
                    throw new errors_1.AppError(400, 'INVALID_TARGET_ROLE', 'HOD can only generate links for TEACHER or STUDENT roles');
                }
            }
            ...
            switch (creatorRole) {
                case shared_1.UserRole.SCHOOL_ADMIN:
                    targetRole = shared_1.UserRole.HOD;
                    break;
                case shared_1.UserRole.HOD:
                    targetRole = shared_1.UserRole.TEACHER;
                    break;
                case shared_1.UserRole.TEACHER:
                    targetRole = shared_1.UserRole.STUDENT;
                    break;
                default:
        ...
        if (creatorRole === shared_1.UserRole.HOD && targetRole === shared_1.UserRole.STUDENT) {
            if (!classId) {
                throw new errors_1.AppError(400, 'CLASS_REQUIRED', 'A class must be selected for student registration links');
            }
            ...
            if (!classRecord) {
                throw new errors_1.AppError(400, 'CLASS_REQUIRED', 'The specified class does not exist');
            }
            if (classRecord.departmentId !== departmentId) {
                throw new errors_1.AppError(403, 'FORBIDDEN', 'Class does not belong to your department');
            }
        }
        // Clamp expiryDays
        ...
        const token = (0, cuid2_1.createId)();
    ...
    async resolveLink(token) {
        const link = await index_1.prisma.registrationLink.findUnique({
            where: { token },
        });
        if (!link) {
            throw new errors_1.AppError(404, 'LINK_NOT_FOUND', 'Registration link not found');
        }
        const now = new Date();
        if (link.expiresAt < now) {
            throw new errors_1.AppError(410, 'LINK_EXPIRED', 'Registration link has expired');
        }
        if (link.useCount >= link.maxUses) {
            throw new errors_1.AppError(410, 'LINK_EXHAUSTED', 'Registration link has reached maximum uses');
        }
        return link;
    }
    /**
    ...
    async registerViaLink(token, data) {
        ...
        if (username) {
            const existingUsername = await index_1.prisma.user.findUnique({
                where: { username },
            });
            if (existingUsername) {
                throw new errors_1.AppError(409, 'DUPLICATE_USERNAME', 'A user with this username already exists');
            }
        }
        ...
        if (admissionNumber) {
            ...
            if (existingUser) {
                throw new errors_1.AppError(409, 'DUPLICATE_ADMISSION', 'A user with this admission number already exists');
            }
        }
        ...
        // Increment useCount
        await index_1.prisma.registrationLink.update({
            where: { id: link.id },
     ...
     */
    async deleteLink(linkId, requesterId, requesterRole, schoolId) {
        // Find the link by id
        const link = await index_1.prisma.registrationLink.findUnique({
            where: { id: linkId },
        });
        ...
        if (!link || link.schoolId !== schoolId) {
            throw new errors_1.AppError(404, 'NOT_FOUND', 'Registration link not found');
        }
        // SCHOOL_ADMIN can delete any link in their school
        if (requesterRole === shared_1.UserRole.SCHOOL_ADMIN) {
            await index_1.prisma.registrationLink.delete({ where: { id: linkId } });
            return;
        }
        // HOD and TEACHER can only delete links they created
        if (requesterRole === shared_1.UserRole.HOD || requesterRole === shared_1.UserRole.TEACHER) {
            if (link.createdById !== requesterId) {
                throw new errors_1.AppError(403, 'FORBIDDEN', 'You can only delete links you created');
            }
            await index_1.prisma.registrationLink.delete({ where: { id: linkId } });
            return;
        }
        // Any other role is forbidden
        throw new errors_1.AppError(403, 'FORBIDDEN', 'You do not have permission to delete registration links');
    }
    /**
     ...
     * Requirements: 4.9
     */
    async addStudentManually(teacherId, schoolId, departmentId, classId, data) {
...
exports.RegistrationLinkService = RegistrationLinkService;
// ─── Singleton Export ─────────────────────────────────────────────────────────
exports.registrationLinkService = new RegistrationLinkService();
//# sourceMappingURL=registrationLinkService.js.map