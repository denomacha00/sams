...
exports.encryptDescriptor = encryptDescriptor;
exports.decryptDescriptor = decryptDescriptor;
exports.deriveSchoolKey = deriveSchoolKey;
 ...
 */
function encryptDescriptor(descriptor, schoolKey) {
    ...
    const cipher = crypto_1.default.createCipheriv(ALGORITHM, schoolKey, iv, {
 ...
 */
function decryptDescriptor(template, schoolKey) {
    if (schoolKey.length !== 32) {
        throw new Error('School key must be exactly 32 bytes for AES-256-GCM');
    }
    // Create decipher with the same IV
    const decipher = crypto_1.default.createDecipheriv(ALGORITHM, schoolKey, template.iv, {
        authTagLength: AUTH_TAG_LENGTH,
    });
 ...
 */
function deriveSchoolKey(schoolId) {
    ...
    const prk = crypto_1.default.createHmac('sha256', salt).update(masterKey).digest();
...
//# sourceMappingURL=biometricEncryption.js.map